#!/usr/bin/env python3
"""
YOLOv8-Pose ONNX 导出脚本
=========================
将 YOLOv8-Pose PyTorch 模型导出为 ONNX 格式，
支持自定义输入尺寸、opset 版本、动态 batch 等参数。

用法示例：
  python export_onnx.py
  python export_onnx.py --model yolov8s-pose.pt --imgsz 640 --opset 11
  python export_onnx.py --model yolov8m-pose.pt --imgsz 416 --dynamic --simplify
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="YOLOv8-Pose ONNX 导出")
    parser.add_argument("--model", type=str, default="yolov8s-pose.pt",
                        help="PyTorch 模型路径 (默认: yolov8s-pose.pt)")
