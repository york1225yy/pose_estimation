"""
Video Swin Transformer — GPU 模式滑动窗口连续行为识别脚本
=========================================================
原理：
...
参数说明:
    --device    计算设备，例如 cuda:0（默认）或 cpu
    --stride    每隔多少帧推理一次（越小越密集，默认 4）
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Video Swin Transformer — GPU 滑动窗口连续行为识别",
    ...
    parser.add_argument(
        ...
        type=str,
        default="cuda:0",
        help="计算设备（默认: cuda:0）。无 GPU 时自动回退到 cpu",
    )
    parser.add_argument(
        "--stride",
        type=int,
        default=4,
        help="推理步长：每收集到 stride 帧新帧后执行一次推理（默认: 4）",
    )
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="result_sliding_gpu.mp4",
        help="输出视频文件路径（默认: result_sliding_gpu.mp4）",
    )
    parser.add_argument(
        "--display",
        action="store_true",
...
def resolve_device(requested: str) -> torch.device:
    """解析设备，CUDA 不可用时自动回退 CPU"""
    if requested.startswith("cuda"):
        if not torch.cuda.is_available():
            print("[警告] CUDA 不可用，自动回退到 CPU 模式。")
            return torch.device("cpu")
        gpu_id = int(requested.split(":")[1]) if ":" in requested else 0
        n = torch.cuda.device_count()
        if gpu_id >= n:
            print(f"[警告] 指定 GPU {gpu_id} 不存在（共 {n} 个），使用 cuda:0。")
            gpu_id = 0
...
def run(args):
    ...
    mode_str = "GPU 加速" if device.type == "cuda" else "CPU（回退）"
    labels = load_labels(args.label)

    print("=" * 62)
    print("  Video Swin Transformer — GPU 滑动窗口连续行为识别")
    print(f"  视频:   {args.video}")
    print(f"  设备:   {device}  [{mode_str}]")
    ...
    win_title = f"滑动窗口行为识别 [{device.type.upper()}]"
    if args.display:
        cv2.namedWindow(win_title, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(win_title, min(width, 1280), min(height, 720))
    ...
    try:
        while True:
            ...
            if frame_idx % 30 == 0:
                ...
                avg_infer = (sum(infer_times) / len(infer_times) * 1000) if infer_times else 0
                print(f"  进度: {frame_idx}/{total}  推理次数: {infer_count}  "
                      f"平均推理: {avg_infer:.0f}ms  已用时: {elapsed:.1f}s", end="\r")
    finally:
        cap.release()
        writer.release()
        if args.display:
    ...
    elapsed = time.time() - t_start
    avg_infer = (sum(infer_times) / len(infer_times) * 1000) if infer_times else 0
    print(f"\n  处理完成: 共 {frame_idx} 帧，推理 {infer_count} 次")
    print(f"  平均单次推理耗时: {avg_infer:.0f} ms")
