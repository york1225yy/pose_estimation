...
用法示例：
  ...
  python infer.py --source video.mp4 --model yolo11s-pose.pt --device cuda:0
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="YOLO11-Pose 驾驶员坐姿推理")
    parser.add_argument("--source", type=str, required=True,
                        help="输入源：图片路径 / 文件夹路径 / 视频路径")
    parser.add_argument("--model", type=str, default="yolo11s-pose.pt",
                        help="YOLO11-Pose 模型权重路径 (默认: yolo11s-pose.pt)")
...
def infer_image(image_path: str, model, classifier, visualizer, args,
    ...
    if not args.no_show:
        if args.display_scale != 1.0:
            ...
            disp = annotated
        cv2.namedWindow("YOLO11-Pose", cv2.WINDOW_NORMAL)  # 支持鼠标拖拽缩放
        cv2.imshow("YOLO11-Pose", disp)
        cv2.waitKey(0)
